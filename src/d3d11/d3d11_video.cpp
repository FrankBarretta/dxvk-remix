...
namespace dxvk {
  ...
  : D3D11DeviceChild<ID3D11VideoProcessorInputView>(pDevice),
    ...
    if (!(dxvkImage->info().usage & VK_IMAGE_USAGE_SAMPLED_BIT)) {
      ...
      dxvkImage = m_copy = pDevice->GetDXVKDevice()->createImage(info, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, DxvkMemoryStats::Category::AppTexture, "D3D11VideoProcessorInputView");
    }
  ...
  : m_ctx(pContext) {
    ...
    m_ubo = Device->createBuffer(bufferInfo, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, DxvkMemoryStats::Category::AppBuffer, "D3D11VideoContext UBO");
  }
