...
namespace dxvk {
  ...
  : m_interface(pInterface), m_device(pDevice), m_dimension(Dimension), m_desc(*pDesc), m_dxgiUsage(DxgiUsage) {
    ...
    if (vkImage == VK_NULL_HANDLE)
      m_image = m_device->GetDXVKDevice()->createImage(imageInfo, memoryProperties, DxvkMemoryStats::Category::AppTexture, "D3D11CommonTexture");
    else
  ...
  D3D11CommonTexture::MappedBuffer D3D11CommonTexture::CreateMappedBuffer(UINT MipLevel) const {
    ...
    MappedBuffer result;
    result.buffer = m_device->GetDXVKDevice()->createBuffer(info, memType, DxvkMemoryStats::Category::AppBuffer, "D3D11CommonTexture Staging");
